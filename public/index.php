...
<?php if ($setupToken !== '' && !$auth->isAuthenticated()): ?>
  <main class="login">
    <section class="card login-card">
      ...
      <p>Choose a password to activate your Banquise account.</p>
      <?php if ($message): ?><div class="alert"><?=h($message)?></div><?php endif; ?>
      <form method="post">
        <input type="hidden" name="form" value="complete_setup">
        <input type="hidden" name="token" value="<?=h($setupToken)?>">
        ...
        <button>Set password and continue</button>
      </form>
      <p class="login-footnote"><a href="/">&larr; Browse the public plugin catalog</a></p>
    </section>
  </main>
<?php elseif (!$auth->isAuthenticated() && $viewLogin): ?>
  <main class="login">
    <section class="card login-card">
      <div class="brand-mark">B</div>
      <h1>Banquise</h1>
      <p>MariaDB plugin fleet control</p>
      <?php if ($message): ?><div class="alert"><?=h($message)?></div><?php endif; ?>
      <form method="post">
...
<?php else: ?>
  <header>
    <a class="brand" href="/">
      ...
      <span>Banquise</span>
      <img class="mariadb-seal" src="/assets/mariadb-foundation-seal.svg" alt="MariaDB Foundation">
    </a>
    <nav>
      ...
      <?php else: ?>
        ...
        <?php else: ?>
          ...
          <?php if ($canViewFleet): ?><a href="#servers">Servers</a><?php endif; ?>
        <?php endif; ?>
        <?php if ($canReviewSubmissions): ?>
          <a class="nav-link-with-badge" href="?page=submissions">Submissions<?php if ($newSubmissionCount): ?><span class="nav-badge"><?=h($newSubmissionCount)?></span><?php endif; ?></a>
        <?php endif; ?>
  ...
  <main class="shell">
    <?php if ($message): ?><div class="alert"><?=nl2br(h($message))?></div><?php endif; ?>
...
<section class="hero">
  <div>
    ...
    <h1><span class="hero-title-blue"><?=h($heroTitle)?></span><br><em><?=h($heroSubtitle)?></em></h1>
    <p><?=h($heroText)?></p>
  </div>
  <div class="metrics<?=count($heroMetrics)<3?' metrics-'.count($heroMetrics):''?>">
    <?php foreach ($heroMetrics as $metric): ?>
      <div><strong><?=h($metric['count'])?></strong><span><?=h($metric['label'])?></span></div>
    <?php endforeach; ?>
  </div>
</section>
...
<?php if ($canViewFleet): ?>
<section id="servers">
  <div class="section-title"><div><span class="eyebrow">FLEET</span><h2>MariaDB servers</h2></div></div>
  <div class="server-grid">
    <?php foreach ($agents as $agent):
        $serverUpdates = $pluginUpdates[$agent['server_uid']] ?? [];
        $serverPluginCount = $pluginCounts[$agent['server_uid']] ?? ['installed' => 0, 'available' => 0];
    ?>
      <a class="server card" href="?agent=<?=urlencode($agent['server_uid'])?>#server-detail">
        <div class="server-top">
          <div class="server-identity">
            <span class="database-icon"><img src="/assets/database-node.svg" alt=""></span>
            <div><span class="server-kicker">DATABASE NODE</span><h3><?=h($agent['display_name'] ?: 'MariaDB server')?></h3></div>
          </div>
          <div class="server-badges">
            <?php if ($serverUpdates):
                $updateTitle = implode("\n", array_map(
                    static fn($update) => $update['name'] . ' ' . $update['installed_version'] . ' → ' . $update['available_version'],
                    $serverUpdates
                ));
            ?>
              <span class="update-available" title="<?=h($updateTitle)?>" aria-label="<?=count($serverUpdates)?> plugin update<?=count($serverUpdates)===1?'':'s'?> available">
                <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3 6.5 8.5l1.8 1.8 2.4-2.4v7.4h2.6V7.9l2.4 2.4 1.8-1.8L12 3Zm-7 13v4h14v-4h-2.5v1.5h-9V16H5Z"/></svg>
                <strong><?=count($serverUpdates)?></strong>
              </span>
            <?php endif; ?>
            <span class="badge <?=h($agent['status'])?>"><?=h($agent['status'])?></span>
          </div>
        </div>
        <div class="server-uid" title="<?=h($agent['server_uid'])?>"><?=h($agent['server_uid'])?></div>
        <div class="server-specs">
          <div><span>MariaDB</span><strong><?=h($agent['mariadb_version'])?></strong></div>
          <div><span>Operating system</span><strong><?=h($agent['os'])?></strong></div>
          <div><span>Architecture</span><strong><?=h($agent['architecture'])?></strong></div>
          <div>
            <span>Plugins</span>
            <strong class="plugin-count" title="<?=h($serverPluginCount['installed'])?> of <?=h($serverPluginCount['available'])?> compatible catalog plugins installed">
              <svg viewBox="0 0 26 26" aria-hidden="true"><path d="M19 13h-2V9h-4V7a2 2 0 1 0-4 0v2H5v4h2a2 2 0 1 1 0 4H5v4h6v-2a2 2 0 1 1 4 0v2h6v-6h-2a1 1 0 0 1 0-2Z"/></svg>
              <span><?=h($serverPluginCount['installed'])?> / <?=h($serverPluginCount['available'])?> installed</span>
            </strong>
          </div>
        </div>
        <div class="server-footer">
          <span class="presence"><span class="dot <?=online($agent,$onlineThreshold)?'up':'down'?>"></span><?=online($agent,$onlineThreshold)?'Online':'Offline'?></span>
          <span>Seen <?=h(str_replace(['T','Z'],[' ',' UTC'],$agent['last_seen_at']))?></span>
          <span><strong><?=h($agent['pending_tasks'])?></strong> pending</span>
        </div>
      </a>
    <?php endforeach; ?>
    <?php if (!$agents): ?><div class="empty card">No agents have registered yet.</div><?php endif; ?>
  </div>
</section>

<?php if ($selected):
    $observed = $app->agentPlugins($selectedUid);
    $tasks = $app->tasks($selectedUid);
    ...
    foreach ($availablePlugins as $availablePlugin) {
        foreach (explode(',', (string)($availablePlugin['plugin_types'] ?? '')) as $type) {
            $type = trim($type);
            if ($type !== '') $operationTypes[strtolower($type)] = $type;
        }
        $maturity = trim((string)($availablePlugin['maturity'] ?? 'unknown')) ?: 'unknown';
        $operationMaturities[strtolower($maturity)] = $maturity;
    }
    asort($operationTypes, SORT_NATURAL | SORT_FLAG_CASE);
    asort($operationMaturities, SORT_NATURAL | SORT_FLAG_CASE);
    $taskWatchAttrs = $hasPendingTasks
        ? ' data-task-watch-url="?task_status=' . h(rawurlencode($selectedUid)) . '" data-task-completion-token="' . h($taskCompletionToken) . '"'
        : '';
?>
<section id="server-detail" class="detail card"<?=$taskWatchAttrs?>>
  <div class="section-title">
    <div>
      <span class="eyebrow">SERVER</span>
      <div class="editable-server-name">
        <h2><?=h($selected['display_name'] ?: $selectedUid)?></h2>
        <?php if ($canManageFleet): ?>
          <button type="button" class="edit-name-button" data-dialog="rename-server-dialog" aria-label="Edit server name" title="Edit server name">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20h4.2L19 9.2 14.8 5 4 15.8V20Zm12-16.2 4.2 4.2 1.1-1.1a1.5 1.5 0 0 0 0-2.1l-2.1-2.1a1.5 1.5 0 0 0-2.1 0L16 3.8Z"/></svg>
          </button>
        <?php endif; ?>
      </div>
      <div class="server-detail-uid">ID: <?=h($selectedUid)?></div>
    </div>
    <?php if ($canManageFleet): ?>
      <div class="server-detail-actions">
        <form method="post">
          <input type="hidden" name="csrf" value="<?=csrf()?>">
          <input type="hidden" name="form" value="agent_status">
          <input type="hidden" name="uid" value="<?=h($selectedUid)?>">
          <button name="status" value="<?=$selected['status']==='active'?'disabled':'active'?>" class="secondary"><?=$selected['status']==='active'?'Disable':'Approve agent'?></button>
        </form>
        <button type="button" class="danger" data-dialog="delete-server-dialog">Delete server</button>
      </div>
    <?php endif; ?>
  </div>
  <?php if ($canManageFleet): ?>
    <form method="post" class="bulk-plugin-form">
      <input type="hidden" name="csrf" value="<?=csrf()?>">
      <input type="hidden" name="form" value="bulk_task">
      <input type="hidden" name="uid" value="<?=h($selectedUid)?>">
      <div class="bulk-heading">
        <div>
          <h3>Plugin operations</h3>
          <p>Select one or more compatible catalog plugins, then apply a single action to the selection.</p>
        </div>
        <div class="bulk-heading-actions">
          <?php if ($availablePlugins): ?>
            <button type="button" class="operation-filter-toggle" data-operation-filter-toggle aria-expanded="false" aria-controls="operation-filters">
              <svg viewBox="0 0 12 12" aria-hidden="true"><path d="m3 4 3 3 3-3Z"/></svg>
              <span>Search and filters</span>
            </button>
          <?php endif; ?>
          <label class="select-all"><input type="checkbox" data-check-all> Select all</label>
        </div>
      </div>
      <?php if ($availablePlugins): ?>
        <div id="operation-filters" class="operation-filters" hidden>
          <label class="operation-search"><span>Search</span><input type="search" data-operation-search placeholder="Search plugins…" autocomplete="off"></label>
          <?php if ($operationTypes): ?>
            <div class="filter-group">
              <span class="filter-label">Type</span>
              <div class="filter-chips">
                <?php foreach ($operationTypes as $value => $label): ?>
                  <button type="button" class="filter-chip" data-operation-filter data-filter-group="type" data-filter-value="<?=h($value)?>" aria-pressed="false"><?=h($label)?></button>
                <?php endforeach; ?>
              </div>
            </div>
          <?php endif; ?>
          <?php if ($operationMaturities): ?>
            <div class="filter-group">
              <span class="filter-label">Maturity</span>
              <div class="filter-chips">
                <?php foreach ($operationMaturities as $value => $label): ?>
                  <button type="button" class="filter-chip" data-operation-filter data-filter-group="maturity" data-filter-value="<?=h($value)?>" aria-pressed="false"><?=h($label)?></button>
                <?php endforeach; ?>
              </div>
            </div>
          <?php endif; ?>
          <div class="filter-summary">
            <span><strong data-operation-visible-count><?=count($availablePlugins)?></strong> of <?=count($availablePlugins)?> plugins</span>
            <button type="button" class="filter-clear" data-operation-filter-clear hidden>Clear filters</button>
          </div>
        </div>
      <?php endif; ?>
          ...
          <tbody>
            <?php foreach ($availablePlugins as $name => $catalogPlugin):
                ...
                $updateAvailable = $installed && $installedVersion !== ''
                    && version_compare(ltrim($installedVersion, 'vV'), ltrim((string)$catalogPlugin['version'], 'vV'), '<');
                $rowTypes = array_values(array_filter(array_map(
                    static fn($type) => strtolower(trim($type)),
                    explode(',', (string)($catalogPlugin['plugin_types'] ?? ''))
                )));
            ...
            ?>
              <tr data-operation-entry data-search-text="<?=h(strtolower($name.' '.($catalogPlugin['plugin_types']??'').' '.$rowMaturity))?>" data-filter-types="<?=h(json_encode($rowTypes,JSON_THROW_ON_ERROR))?>" data-filter-maturity="<?=h($rowMaturity)?>">
                ...
                <td><?=h($installedVersion!==''?$installedVersion:'—')?></td>
              </tr>
            <?php endforeach; ?>
            ...
            <?php if (!$availablePlugins): ?><tr><td colspan="6" class="empty-row">No catalog plugins are compatible with this server.</td></tr><?php endif; ?>
          </tbody>
        </table>
      </div>
      <div class="bulk-action-bar">
        <label>Action<select name="action"><option value="install">Install and load</option><option value="update">Update</option><option value="load">Load</option><option value="uninstall">Uninstall</option></select></label>
        <div class="selection-count"><strong data-selection-count>0</strong> selected</div>
        <button <?=!$availablePlugins?'disabled':''?>>Queue action for selected plugins</button>
      </div>
    </form>
  <?php else: ?>
      ...
      <tbody>
        <?php foreach ($availablePlugins as $name => $catalogPlugin): $state = $observedByName[$name] ?? null; $installed = !empty($state['installed']); ?>
          <tr>
            ...
            <td><span class="state-mark <?=$installed?'yes':'no'?>"><?=$installed?'Installed':'Not installed'?></span></td>
            <td><span class="state-mark <?=!empty($state['loaded'])?'yes':'no'?>"><?=!empty($state['loaded'])?'Loaded':'Not loaded'?></span></td>
            <td><?=h(trim((string)($state['installed_version']??'')) ?: '—')?></td>
          </tr>
        <?php endforeach; ?>
        <?php if (!$availablePlugins): ?><tr><td colspan="5" class="empty-row">No catalog plugins are compatible with this server.</td></tr><?php endif; ?>
      </tbody>
    </table>
  <?php endif; ?>
  <h3>Recent tasks</h3>
  <table>
    <thead><tr><th>ID</th><th>Action</th><th>Plugin</th><th>State</th><th>Result</th></tr></thead>
    <tbody>
...
<?php if ($canManageFleet): ?>
  <dialog id="rename-server-dialog">
    <form method="post" class="stack">
      <div class="dialog-title"><h2>Edit server name</h2><button type="button" class="icon" data-close>×</button></div>
      <input type="hidden" name="csrf" value="<?=csrf()?>">
      <input type="hidden" name="form" value="agent_rename">
      <input type="hidden" name="uid" value="<?=h($selectedUid)?>">
      <label>Display name<input name="display_name" value="<?=h($selected['display_name'] ?: 'MariaDB server')?>" maxlength="100" autocomplete="off" required autofocus></label>
      <small>The name is only a friendly label. The immutable server ID remains <?=h($selectedUid)?>.</small>
      <button>Save server name</button>
    </form>
  </dialog>
  <dialog id="delete-server-dialog">
    <form method="post" class="stack">
      <div class="dialog-title"><h2>Delete <?=h($selected['display_name'] ?: $selectedUid)?></h2><button type="button" class="icon" data-close>×</button></div>
      <input type="hidden" name="csrf" value="<?=csrf()?>">
      <input type="hidden" name="form" value="agent_delete">
      <input type="hidden" name="uid" value="<?=h($selectedUid)?>">
      <p>This permanently removes the server registration, its observed plugin state, and all task history. If the agent still has valid credentials, its next request will be rejected; registering it again requires a new enrollment.</p>
      <label>Type DELETE to confirm<input name="confirmation" pattern="DELETE" autocomplete="off" required></label>
      <button class="danger">Delete server permanently</button>
    </form>
  </dialog>
<?php endif; ?>
<?php endif; ?>
...
foreach (($catalog['plugins'] ?? []) as $catalogPlugin) {
    foreach (explode(',', (string)($catalogPlugin['plugin_types'] ?? '')) as $type) {
        $type = trim($type);
        if ($type !== '') $catalogTypes[strtolower($type)] = $type;
    }
    $maturity = trim((string)($catalogPlugin['maturity'] ?? 'unknown')) ?: 'unknown';
    $catalogMaturities[strtolower($maturity)] = $maturity;
}
asort($catalogTypes, SORT_NATURAL | SORT_FLAG_CASE);
asort($catalogMaturities, SORT_NATURAL | SORT_FLAG_CASE);
...
<section id="catalog" data-catalog-filter-root>
  <div class="section-title">
    <div>
      <span class="eyebrow">SIGNED ARTIFACTS</span>
      <div class="catalog-title"><h2>Plugin catalog</h2><?php if ($catalogMode === 'remote'): ?><span class="badge neutral" title="Mirrored verbatim from an external catalog; local additions, edits, and updates are disabled.">External catalog</span><?php endif; ?><button type="button" class="catalog-copy-button" data-copy-catalog-url="<?=h($catalogPublicUrl)?>" title="Copy catalog URL" aria-label="Copy catalog URL"><svg class="copy-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 7V4.8C8 3.8 8.8 3 9.8 3h9.4c1 0 1.8.8 1.8 1.8v9.4c0 1-.8 1.8-1.8 1.8H17v2.2c0 1-.8 1.8-1.8 1.8H5.8c-1 0-1.8-.8-1.8-1.8V8.8C4 7.8 4.8 7 5.8 7H8Zm2 0h5.2c1 0 1.8.8 1.8 1.8V14h2V5h-9v2Zm-4 2v9h9V9H6Z"/></svg><svg class="copy-done-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="m9.2 16.2-4.4-4.4 1.8-1.8 2.6 2.6 8.2-8.2 1.8 1.8-10 10Z"/></svg></button><?php if ($catalogKeyId !== ''): ?><a class="catalog-key-button" href="/catalog.pub" download="catalog.pub" title="Download catalog public key" aria-label="Download catalog public key"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M11 3h2v10.2l3.6-3.6 1.4 1.4-6 6-6-6 1.4-1.4L11 13.2V3ZM5 19h14v2H5v-2Z"/></svg></a><?php endif; ?></div>
    </div>
    <?php if ($canEditCatalogNow): ?>
      <div class="catalog-header-actions">
        <?php if ($authorityActive): ?>
          <form method="post" data-spin-on-submit>
            <input type="hidden" name="csrf" value="<?=csrf()?>">
            <input type="hidden" name="form" value="authority_verify">
            <button type="submit" class="catalog-refresh-button catalog-verify-button status-<?=h($authorityOverallStatus)?>" title="Verify the catalog against the authority" aria-label="Verify against authority">
              <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2 4 5v6c0 5 3.4 9.4 8 11 4.6-1.6 8-6 8-11V5l-8-3Zm-1.2 14L6.8 12l1.4-1.4 2.6 2.6 5.6-5.6 1.4 1.4-7 7Z"/></svg>
            </button>
          </form>
        <?php endif; ?>
        <form method="post" data-spin-on-submit>
          <input type="hidden" name="csrf" value="<?=csrf()?>">
          <input type="hidden" name="form" value="catalog_check_updates">
          <button type="submit" class="catalog-refresh-button" title="Check every catalog plugin's repository for a newer GitHub release" aria-label="Check for plugin updates">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5V2L8 6l4 4V7c2.76 0 5 2.24 5 5 0 .65-.13 1.26-.35 1.83l1.46 1.46A6.93 6.93 0 0 0 19 12c0-3.87-3.13-7-7-7Zm0 12c-2.76 0-5-2.24-5-5 0-.65.13-1.26.35-1.83L5.89 8.71A6.93 6.93 0 0 0 5 12c0 3.87 3.13 7 7 7v3l4-4-4-4v3Z"/></svg>
          </button>
        </form>
        <button type="button" data-dialog="repo-dialog">Add GitHub repository</button>
      </div>
    <?php elseif ($canWriteCatalog): ?>
      <p class="form-note">This catalog is mirrored from an external source; local catalog management is disabled. <?php if ($canManageCatalogSource): ?><a href="?page=admin#admin">Change the catalog source</a>.<?php endif; ?></p>
    <?php elseif (!$auth->isAuthenticated()): ?>
      <button type="button" data-dialog="submit-plugin-dialog">Submit a plugin</button>
    <?php endif; ?>
  </div>
  <?php if ($legacyDuplicates): ?>
    <div class="alert legacy-duplicates-alert">Found <?=count($legacyDuplicates)?> catalog entr<?=count($legacyDuplicates)===1?'y':'ies'?> that predate OS tracking and now duplicate a newer, OS-tagged entry for the same plugin — leftover from before Banquise could tell them apart. <button type="button" class="small secondary" data-dialog="prune-duplicates-dialog">Review and clean up</button></div>
  <?php endif; ?>
  <?php if ($authorityMissing): ?>
    <div class="alert authority-missing-alert">Found <?=count($authorityMissing)?> plugin build<?=count($authorityMissing)===1?'':'s'?> in the authority catalog that <?=count($authorityMissing)===1?'isn\'t':'aren\'t'?> in yours yet. <button type="button" class="small secondary" data-dialog="authority-missing-dialog">Review and add</button></div>
  <?php endif; ?>
      ...
      <div class="catalog-filter-top">
        <?php if ($catalogTypes): ?>
          <div class="filter-group">
            <span class="filter-label">Type</span>
            <div class="filter-chips">
              <?php foreach ($catalogTypes as $value => $label): ?>
                <button type="button" class="filter-chip" data-catalog-filter data-filter-group="type" data-filter-value="<?=h($value)?>" aria-pressed="false"><?=h($label)?></button>
              <?php endforeach; ?>
            </div>
          </div>
        <?php endif; ?>
        <label class="catalog-search"><span>Search</span><input type="search" data-catalog-search placeholder="Search plugins…" autocomplete="off"></label>
      </div>
      <?php if ($catalogMaturities): ?>
        <div class="filter-group">
          <span class="filter-label">Maturity</span>
          <div class="filter-chips">
            <?php foreach ($catalogMaturities as $value => $label): ?>
              <button type="button" class="filter-chip" data-catalog-filter data-filter-group="maturity" data-filter-value="<?=h($value)?>" aria-pressed="false"><?=h($label)?></button>
            <?php endforeach; ?>
          </div>
        </div>
      <?php endif; ?>
      <div class="filter-summary">
        <span><strong data-catalog-visible-count><?=count($catalogGroups)?></strong> of <?=count($catalogGroups)?> plugins</span>
        <button type="button" class="filter-clear" data-catalog-filter-clear hidden>Clear filters</button>
      </div>
    </div>
  <?php endif; ?>
  <div class="catalog-list">
    <?php foreach ($catalogGroups as $variants):
        $first = $variants[0];
        ...
        foreach ($variants as $variant) {
            foreach (explode(',', (string)($variant['plugin_types'] ?? '')) as $type) {
                ...
                if ($type !== '') $filterTypes[$type] = true;
            }
            $searchParts[] = implode(' ', [
                $variant['name'] ?? '', $variant['description'] ?? '', $variant['plugin_types'] ?? '',
                $variant['maturity'] ?? '', $variant['license'] ?? '', $variant['repository'] ?? '',
                $variant['os'] ?? '', $variant['author'] ?? '',
            ]);
        }
        ...
        $hasUpdate = false;
        foreach ($variants as $variant) {
            if (isset($updateAvailability[$app->catalogEntryId($variant)])) { $hasUpdate = true; break; }
        }
    ...
    ?>
      <article class="plugin card" data-catalog-entry data-search-text="<?=h($catalogSearchText)?>" data-filter-types="<?=h(json_encode($filterTypes,JSON_THROW_ON_ERROR))?>" data-filter-maturity="<?=h($filterMaturity)?>">
        <?php if ($hasUpdate): ?><span class="plugin-update-badge">Update available</span><?php endif; ?>
        <?php if ($authorityActive): ?><?=authorityBadge($authorityCardStatus??'local')?><?php endif; ?>
        <div>
          <h3><a class="plugin-repository-link" href="<?=h($first['repository'])?>" target="_blank" rel="noopener noreferrer"><?=h($first['name'])?><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 4h6v6h-2V7.4l-7.3 7.3-1.4-1.4L16.6 6H14V4ZM5 6h6v2H7v9h9v-4h2v6H5V6Z"/></svg><span class="visually-hidden"> repository</span></a></h3>
          <p><?=h($first['description']??'')?></p>
          <?php if ($canEditCatalogNow && !$isMulti): ?>
            <div class="plugin-actions">
              <button type="button" class="small secondary" data-dialog="edit-<?=h($groupEntryId)?>">Edit</button>
              <?php if ($authorityCardStatus === 'differs'): ?><?=authoritySyncButton($groupEntryId)?><?php endif; ?>
              <button type="button" class="small secondary" data-dialog="refresh-<?=h($groupEntryId)?>">Refresh from GitHub</button>
              <button type="button" class="small danger" data-dialog="delete-<?=h($groupEntryId)?>">Delete</button>
            </div>
          <?php endif; ?>
        </div>
        ...
        <?php if ($isMulti): ?>
          <dl>
            <div><dt>MariaDB versions</dt><dd><?=h(implode(', ',$mariadbVersions))?></dd></div>
            <?php if ($osValues): ?><div><dt>OS builds</dt><dd><?=h(implode(', ',$osValues))?></dd></div><?php endif; ?>
            <div><dt>Type</dt><dd><?=h($first['plugin_types']??'')?></dd></div>
            <?php if (count($architectures) > 1): ?><div><dt>Architectures</dt><dd><?=h(implode(', ',$architectures))?></dd></div><?php endif; ?>
            <?php if (!empty($first['author'])): ?><div><dt>Author</dt><dd><a href="https://github.com/<?=h($first['author'])?>" target="_blank" rel="noopener noreferrer"><?=h($first['author'])?></a></dd></div><?php endif; ?>
            <?php if ($auth->isAuthenticated()): ?>
              <div><dt>Installed on</dt><dd><?php if ($canViewFleet): ?><button type="button" class="installation-count" <?=$pluginServers?'data-dialog="installed-'.$groupEntryId.'"':'disabled'?>><strong><?=count($pluginServers)?></strong> server<?=count($pluginServers)===1?'':'s'?></button><?php else: ?><?=count($pluginServers)?> server<?=count($pluginServers)===1?'':'s'?><?php endif; ?></dd></div>
            <?php else: ?>
              <div aria-hidden="true"></div>
            <?php endif; ?>
            <div><dt>Maturity</dt><dd><span class="badge neutral"><?=h($first['maturity']??'unknown')?></span></dd></div>
          </dl>

          <details class="plugin-variants">
            <summary><?=count($variants)?> build variants</summary>
            <div class="variant-table-wrap">
              <table class="variant-table">
                <thead>
                  <tr>
                    <th>Version</th><th>MariaDB</th><th>Architecture</th><th>OS</th><th>Last release</th><th>Last commit</th>
                    <?php if ($authorityActive): ?><th>Authority</th><?php endif; ?>
                    <?php if ($canEditCatalogNow): ?><th></th><?php endif; ?>
                  </tr>
                </thead>
                <tbody>
                  <?php foreach ($variants as $variant):
                      $variantEntryId = $app->catalogEntryId($variant);
                      $variantAuthorityStatus = $authorityStatus[$variantEntryId] ?? null;
                  ?>
                    <tr>
                      <td><?=h($variant['version'])?></td>
                      <td><?=h($variant['mariadb_version'])?></td>
                      <td><?=h($variant['architecture'])?></td>
                      <td><?=h($variant['os']??'—')?></td>
                      <td><?php if (!empty($variant['last_release_at'])): ?><span title="<?=h(fullDate($variant['last_release_at']))?>"><?=h(relativeTime($variant['last_release_at']))?></span><?php else: ?>—<?php endif; ?></td>
                      <td><?php if (!empty($variant['last_commit_at'])): ?><span title="<?=h(fullDate($variant['last_commit_at']))?>"><?=h(relativeTime($variant['last_commit_at']))?></span><?php else: ?>—<?php endif; ?></td>
                      <?php if ($authorityActive): ?><td><?=authorityBadge($variantAuthorityStatus??'local')?></td><?php endif; ?>
                      <?php if ($canEditCatalogNow): ?>
                        <td>
                          <div class="variant-actions">
                            <button type="button" class="small secondary" data-dialog="edit-<?=h($variantEntryId)?>">Edit</button>
                            <?php if ($variantAuthorityStatus === 'differs'): ?><?=authoritySyncButton($variantEntryId)?><?php endif; ?>
                            <button type="button" class="small secondary" data-dialog="refresh-<?=h($variantEntryId)?>">Refresh</button>
                            <button type="button" class="small danger" data-dialog="delete-<?=h($variantEntryId)?>">Delete</button>
                          </div>
                        </td>
                      <?php endif; ?>
                    </tr>
        ...
        <?php else: ?>
          <dl>
            <div><dt>Version</dt><dd><?=h($first['version'])?></dd></div>
            <div><dt>MariaDB</dt><dd><?=h($first['mariadb_version'])?></dd></div>
            <div><dt>Architecture</dt><dd><?=h($first['architecture'])?></dd></div>
            <?php if (!empty($first['os'])): ?><div><dt>OS</dt><dd><?=h($first['os'])?></dd></div><?php endif; ?>
            <div><dt>Type</dt><dd><?=h($first['plugin_types']??'')?></dd></div>
            <?php if (!empty($first['author'])): ?><div><dt>Author</dt><dd><a href="https://github.com/<?=h($first['author'])?>" target="_blank" rel="noopener noreferrer"><?=h($first['author'])?></a></dd></div><?php endif; ?>
            <?php if (!empty($first['last_release_at'])): ?><div><dt>Last release</dt><dd><span title="<?=h(fullDate($first['last_release_at']))?>"><?=h(relativeTime($first['last_release_at']))?></span></dd></div><?php endif; ?>
            <?php if (!empty($first['last_commit_at'])): ?><div><dt>Last commit</dt><dd><span title="<?=h(fullDate($first['last_commit_at']))?>"><?=h(relativeTime($first['last_commit_at']))?></span></dd></div><?php endif; ?>
            <?php if ($auth->isAuthenticated()): ?>
              <div><dt>Installed on</dt><dd><?php if ($canViewFleet): ?><button type="button" class="installation-count" <?=$pluginServers?'data-dialog="installed-'.$groupEntryId.'"':'disabled'?>><strong><?=count($pluginServers)?></strong> server<?=count($pluginServers)===1?'':'s'?></button><?php else: ?><?=count($pluginServers)?> server<?=count($pluginServers)===1?'':'s'?><?php endif; ?></dd></div>
            <?php else: ?>
              <div aria-hidden="true"></div>
      ...
      <?php if ($canViewFleet && $pluginServers): ?>
        <dialog id="installed-<?=h($groupEntryId)?>" class="installation-dialog">
          <div class="dialog-content">
            <div class="dialog-title"><div><span class="eyebrow">INSTALLATIONS</span><h2><?=h($first['name'])?></h2></div><button type="button" class="icon" data-close>×</button></div>
            <p>Servers currently reporting this plugin as installed, regardless of installed version or which build variant.</p>
            <div class="installation-list">
              <?php foreach ($pluginServers as $pluginServer): ?>
                <a href="?agent=<?=rawurlencode($pluginServer['server_uid'])?>#server-detail" class="installation-server">
                  <span class="database-icon"><img src="/assets/database-node.svg" alt=""></span>
                  <span class="installation-server-name"><strong><?=h($pluginServer['display_name']?:$pluginServer['server_uid'])?></strong><small><?=h($pluginServer['server_uid'])?></small></span>
                  <span><small>Installed version</small><strong><?=h($pluginServer['installed_version']?:'Unknown')?></strong></span>
                  <span><small>Runtime</small><strong class="presence"><span class="dot <?=!empty($pluginServer['loaded'])?'up':'down'?>"></span><?=!empty($pluginServer['loaded'])?'Loaded':'Not loaded'?></strong></span>
                  <span class="badge <?=h($pluginServer['status'])?>"><?=h($pluginServer['status'])?></span>
                </a>
              <?php endforeach; ?>
            </div>
          </div>
      ...
      <?php if ($canEditCatalogNow): foreach ($variants as $variant): $variantEntryId = $app->catalogEntryId($variant); ?>
        <dialog id="edit-<?=h($variantEntryId)?>">
          <form method="post" class="stack catalog-editor">
            <div class="dialog-title"><h2>Edit <?=h($variant['name'])?></h2><button type="button" class="icon" data-close>×</button></div>
            <input type="hidden" name="csrf" value="<?=csrf()?>">
            <input type="hidden" name="form" value="catalog_edit">
            <input type="hidden" name="entry_id" value="<?=h($variantEntryId)?>">
            <div class="form-grid">
              <label>Name<input name="name" value="<?=h($variant['name'])?>" required></label>
              <label>Version<input name="version" value="<?=h($variant['version'])?>" required></label>
              <label>MariaDB version<input name="mariadb_version" value="<?=h($variant['mariadb_version'])?>" required></label>
              <label>Architecture<input name="architecture" value="<?=h($variant['architecture'])?>" required></label>
              <label>OS<input name="os" value="<?=h($variant['os']??'')?>" placeholder="linux, el8, ubuntu24.04…"></label>
              <label>Soname<input name="soname" value="<?=h($variant['soname'])?>" required></label>
              <label>Plugin types<input name="plugin_types" value="<?=h($variant['plugin_types']??'')?>"></label>
              <label>License<input name="license" value="<?=h($variant['license']??'')?>"></label>
              <label>Maturity<input name="maturity" value="<?=h($variant['maturity']??'')?>"></label>
              <label>Author<input name="author" value="<?=h($variant['author']??'')?>"></label>
            </div>
            <label>Repository<input type="url" name="repository" value="<?=h($variant['repository'])?>" required></label>
            <label>Download URL<input type="url" name="download_url" value="<?=h($variant['download_url'])?>" required></label>
            <label>SHA-256<input name="sha256" value="<?=h($variant['sha256'])?>" minlength="64" maxlength="64" required></label>
            <div class="form-grid">
              <label>Archive type<input name="archive_type" value="<?=h($variant['archive_type']??'')?>" placeholder="tar.gz"></label>
              <label>Archive member<input name="archive_member" value="<?=h($variant['archive_member']??'')?>"></label>
            </div>
            <label>Description<input name="description" value="<?=h($variant['description']??'')?>"></label>
            <label>Dependencies<input name="dependencies" value="<?=h($variant['dependencies']??'')?>"></label>
            <label>Install message<textarea name="message"><?=h($variant['message']??'')?></textarea></label>
            <label>Minisign key password<input type="password" name="signing_password" autocomplete="current-password" required><small>The live catalog changes only after signing and verification succeed.</small></label>
            <button>Save, sign, and publish</button>
          </form>
        </dialog>

        <dialog id="refresh-<?=h($variantEntryId)?>">
          <form method="post" class="stack">
            <div class="dialog-title"><h2>Refresh <?=h($variant['name'])?></h2><button type="button" class="icon" data-close>×</button></div>
            <input type="hidden" name="csrf" value="<?=csrf()?>">
            <input type="hidden" name="form" value="catalog_refresh">
            <input type="hidden" name="entry_id" value="<?=h($variantEntryId)?>">
            <p>Fetch the latest GitHub release, recalculate its checksum and archive member, and re-detect plugin types, maturity, license, and description—even when the release version has not changed. If the release now has OS/architecture builds that weren't in the catalog before, they're added too, not just this entry.</p>
            <label>Minisign key password<input type="password" name="signing_password" autocomplete="current-password" required></label>
            <button>Refresh, sign, and publish</button>
          </form>
        </dialog>

        <dialog id="delete-<?=h($variantEntryId)?>">
          <form method="post" class="stack">
            <div class="dialog-title"><h2>Delete <?=h($variant['name'])?></h2><button type="button" class="icon" data-close>×</button></div>
            <input type="hidden" name="csrf" value="<?=csrf()?>">
            <input type="hidden" name="form" value="catalog_delete">
            <input type="hidden" name="entry_id" value="<?=h($variantEntryId)?>">
            <p>This removes only this MariaDB version, architecture, and soname entry. Installed plugins on agents are not uninstalled automatically.</p>
            <label>Type DELETE to confirm<input name="confirmation" pattern="DELETE" autocomplete="off" required></label>
            <label>Minisign key password<input type="password" name="signing_password" autocomplete="current-password" required></label>
            <button class="danger">Delete, sign, and publish</button>
          </form>
        </dialog>
        <?php if (($authorityStatus[$variantEntryId] ?? null) === 'differs'): ?>
          <dialog id="authority-sync-<?=h($variantEntryId)?>">
            <form method="post" class="stack">
              <div class="dialog-title"><h2>Sync <?=h($variant['name'])?> from the authority</h2><button type="button" class="icon" data-close>×</button></div>
              <input type="hidden" name="csrf" value="<?=csrf()?>">
              <input type="hidden" name="form" value="authority_sync_entry">
              <input type="hidden" name="entry_id" value="<?=h($variantEntryId)?>">
              <p>Replaces this entry's version, checksum, and other fields with the authority catalog's copy, then signs and publishes with this instance's own key.</p>
              <label>Minisign key password<input type="password" name="signing_password" autocomplete="current-password" required></label>
              <button>Sync, sign, and publish</button>
            </form>
          </dialog>
        <?php endif; ?>
      <?php endforeach; endif; ?>
    <?php endforeach; ?>
    <div class="catalog-no-matches card" data-catalog-no-matches hidden>No catalog entries match the selected filters.</div>
  </div>
</section>
      ...
      <div class="admin-grid">
        <?php if ($canManageEnrollment): ?>
          <article class="admin-panel card">
            ...
            <p>Enrollment tokens authorize only the initial registration. After registration, each agent authenticates with its own generated credential.</p>
            <form method="post" class="stack">
              <input type="hidden" name="csrf" value="<?=csrf()?>">
              <input type="hidden" name="form" value="enrollment_mode">
              <div class="mode-options">
                <label class="mode-option"><input type="radio" name="mode" value="dedicated" <?=$enrollmentMode==='dedicated'?'checked':''?>><span><strong>Dedicated one-time tokens</strong><small>Each token registers one server and is then removed.</small></span></label>
                <label class="mode-option"><input type="radio" name="mode" value="shared" <?=$enrollmentMode==='shared'?'checked':''?>><span><strong>Shared token</strong><small>Use the enrollment token hash configured in config.php.</small></span></label>
              </div>
              <button>Save enrollment mode</button>
            </form>
          </article>
          <article class="admin-panel card">
            <div class="admin-panel-title">
              <div><h3>Available dedicated tokens</h3><p><?=count($enrollmentTokens)?> of 10 available</p></div>
              <?php if ($enrollmentMode === 'dedicated' && count($enrollmentTokens) < 10): ?>
                <form method="post" class="token-generator">
                  <input type="hidden" name="csrf" value="<?=csrf()?>">
                  <input type="hidden" name="form" value="enrollment_generate">
                  <label>Generate<select name="count"><?php for ($i = 1; $i <= 10 - count($enrollmentTokens); ++$i): ?><option value="<?=$i?>"><?=$i?></option><?php endfor; ?></select></label>
            ...
            <div class="token-list">
              <?php foreach ($enrollmentTokens as $enrollmentToken): ?>
                <div class="token-row">
                  <div><code><?=h($enrollmentToken['token'])?></code><small>Created <?=h(str_replace(['T','Z'],[' ',' UTC'],$enrollmentToken['created_at']))?></small></div>
                  <div class="token-actions">
                    <button type="button" class="small secondary" data-copy-token="<?=h($enrollmentToken['token'])?>">Copy</button>
                    <form method="post">
                      <input type="hidden" name="csrf" value="<?=csrf()?>">
                      <input type="hidden" name="form" value="enrollment_revoke">
                      <input type="hidden" name="token_id" value="<?=h($enrollmentToken['id'])?>">
                      <button class="small danger">Revoke</button>
                    </form>
                  </div>
                </div>
              <?php endforeach; ?>
              <?php if (!$enrollmentTokens): ?><div class="empty-token-list">No dedicated enrollment tokens are available.</div><?php endif; ?>
            </div>
          </article>
        <?php endif; ?>
        <?php if ($canManageCatalogSource): ?>
          <article class="admin-panel card">
            ...
            <p>Curate the catalog here, or mirror another Banquise (or compatible) instance's already-signed catalog verbatim instead.</p>
            <form method="post" class="stack" enctype="multipart/form-data">
              <input type="hidden" name="csrf" value="<?=csrf()?>">
              <input type="hidden" name="form" value="catalog_source">
              <div class="mode-options">
              ...
              <button>Save catalog source</button>
            </form>
            <?php if ($catalogMode === 'remote'): ?>
              ...
              <form method="post" data-spin-on-submit>
                <input type="hidden" name="csrf" value="<?=csrf()?>">
                <input type="hidden" name="form" value="catalog_source_sync">
                <button type="submit" class="small secondary" data-loading-label="Syncing…">Sync now</button>
              </form>
            <?php endif; ?>
          </article>
        <?php endif; ?>
        <?php if ($canManageCatalogSource && $catalogMode === 'local'): ?>
          <article class="admin-panel card">
            ...
            <p>Optionally cross-check this catalog against another Banquise's — a difference or a plugin only the authority has is flagged, never applied automatically.</p>
            <form method="post" class="stack" enctype="multipart/form-data">
              <input type="hidden" name="csrf" value="<?=csrf()?>">
              <input type="hidden" name="form" value="authority_source">
              ...
              <button>Save authority source</button>
            </form>
        ...
        <?php if ($canManageDistribution && $catalogMode === 'local'): ?>
          <article class="admin-panel card">
            ...
            <p>How MariaDB servers fetch plugin files referenced by the catalog.</p>
            <form method="post" class="stack">
              <input type="hidden" name="csrf" value="<?=csrf()?>">
              <input type="hidden" name="form" value="distribution_mode">
              <div class="mode-options">
                <label class="mode-option"><input type="radio" name="mode" value="public" <?=$distributionMode==='public'?'checked':''?>><span><strong>Public</strong><small>download_url points at each plugin's original GitHub release asset.</small></span></label>
                <label class="mode-option"><input type="radio" name="mode" value="private" <?=$distributionMode==='private'?'checked':''?>><span><strong>Private</strong><small>Banquise downloads and mirrors every plugin file; servers fetch it from this Banquise instance instead of the internet.</small></span></label>
              </div>
              <label>Minisign key password<input type="password" name="signing_password" autocomplete="current-password"><small>Only needed if the catalog isn't empty — switching modes re-signs and republishes it so every entry matches immediately.</small></label>
              <button>Save distribution mode</button>
            </form>
          </article>
        ...
        <?php if ($canManageUsers): ?>
          <article class="admin-panel card users-panel">
            <h3>Create a user</h3>
            <p>New users receive an emailed setup link to choose their own password. A user can hold multiple roles.</p>
            <form method="post" class="stack">
              <input type="hidden" name="csrf" value="<?=csrf()?>">
              ...
              <div class="role-checkboxes">
                <?php foreach (BanquiseAuth::ROLES as $role): ?>
                  <label class="role-checkbox"><input type="checkbox" name="roles[]" value="<?=h($role)?>"><span><?=h(roleLabel($role))?></span></label>
                <?php endforeach; ?>
              </div>
            ...
            <div class="user-list">
              <?php foreach ($users as $user): ?>
                <div class="user-row">
                  ...
                  <form method="post" class="stack user-roles-form">
                    <input type="hidden" name="csrf" value="<?=csrf()?>">
                    <input type="hidden" name="form" value="user_roles">
                    <input type="hidden" name="user_id" value="<?=h($user['id'])?>">
                    <div class="role-checkboxes">
                      <?php foreach (BanquiseAuth::ROLES as $role): ?>
                        <label class="role-checkbox"><input type="checkbox" name="roles[]" value="<?=h($role)?>" <?=in_array($role,$user['roles'],true)?'checked':''?>><span><?=h(roleLabel($role))?></span></label>
                      <?php endforeach; ?>
                    </div>
                    <button class="small secondary">Save roles</button>
                  </form>
                  <div class="user-row-actions">
                    <form method="post">
                      <input type="hidden" name="csrf" value="<?=csrf()?>">
                      <input type="hidden" name="form" value="user_status">
                      <input type="hidden" name="user_id" value="<?=h($user['id'])?>">
                      <input type="hidden" name="status" value="<?=$user['status']==='active'?'disabled':'active'?>">
                      <button class="small secondary"><?=$user['status']==='active'?'Disable':'Enable'?></button>
                    </form>
                    <?php if (!$user['has_password']): ?>
                      <form method="post">
                        <input type="hidden" name="csrf" value="<?=csrf()?>">
                        <input type="hidden" name="form" value="user_resend_setup">
                        <input type="hidden" name="user_id" value="<?=h($user['id'])?>">
                        <button class="small secondary">Resend setup link</button>
                      </form>
                    <?php endif; ?>
                  </div>
                </div>
              <?php endforeach; ?>
              <?php if (!$users): ?><div class="empty-token-list">No other users yet.</div><?php endif; ?>
            </div>
          </article>
        <?php endif; ?>
      </div>
...
<?php elseif ($isSubmissionsPage): ?>
  ...
  ?>
    <section id="submissions">
      <div class="section-title"><div><span class="eyebrow">COMMUNITY</span><h2>Plugin submissions</h2></div></div>
      <nav class="status-filters">
        <a class="badge<?=$statusFilter===''?' active-filter':''?>" href="?page=submissions">All</a>
        <?php foreach ($allowedStatuses as $statusOption): ?>
          <a class="badge <?=h($statusOption)?><?=$statusFilter===$statusOption?' active-filter':''?>" href="?page=submissions&status=<?=urlencode($statusOption)?>"><?=h(submissionStatusLabel($statusOption))?></a>
        <?php endforeach; ?>
      </nav>
      <table>
        <thead><tr><th>Name</th><th>Repository</th><th>Submitted by</th><th>Status</th><th>Comments</th><th>Submitted</th><th></th></tr></thead>
        <tbody>
          <?php foreach ($submissions as $submission): ?>
            <tr>
              <td><strong><?=h($submission['name'] ?: '—')?></strong></td>
              <td><a href="<?=h($submission['repository'])?>" target="_blank" rel="noopener noreferrer"><?=h($submission['repository'])?></a></td>
              <td><?=h($submission['submitter_name'] ?: '—')?><?php if ($submission['submitter_email']): ?><br><small><?=h($submission['submitter_email'])?></small><?php endif; ?></td>
              <td><span class="badge <?=h($submission['status'])?>"><?=h(submissionStatusLabel($submission['status']))?></span></td>
              <td><?=h($submission['comment_count'])?></td>
              <td><?=h(str_replace(['T','Z'],[' ',' UTC'],$submission['created_at']))?></td>
              <td>
                <div class="submission-row-actions">
                  <button type="button" class="small secondary" data-dialog="submission-<?=h($submission['id'])?>">Review</button>
                  <?php if ($canEditCatalogNow && $submission['status'] === 'reviewed_ok'): ?><a href="?submission_id=<?=h($submission['id'])?>#catalog" class="as-button">Add to catalog</a><?php endif; ?>
                </div>
              </td>
            </tr>
          <?php endforeach; ?>
    ...
    <?php foreach ($submissions as $submission): $comments = $app->submissionComments((int)$submission['id']); ?>
      <dialog id="submission-<?=h($submission['id'])?>" class="submission-dialog">
        <div class="dialog-content">
          <div class="dialog-title"><div><span class="eyebrow">SUBMISSION #<?=h($submission['id'])?></span><h2><?=h($submission['name'] ?: 'Untitled plugin')?></h2></div><button type="button" class="icon" data-close>×</button></div>
          <dl class="submission-fields">
            <div><dt>Repository</dt><dd><a href="<?=h($submission['repository'])?>" target="_blank" rel="noopener noreferrer"><?=h($submission['repository'])?></a></dd></div>
            <div><dt>Description</dt><dd><?=h($submission['description'] ?: '—')?></dd></div>
            <div><dt>Plugin types</dt><dd><?=h($submission['plugin_types'] ?: '—')?></dd></div>
            <div><dt>License</dt><dd><?=h($submission['license'] ?: '—')?></dd></div>
            <div><dt>Maturity</dt><dd><?=h($submission['maturity'] ?: '—')?></dd></div>
            <div><dt>Dependencies</dt><dd><?=h($submission['dependencies'] ?: '—')?></dd></div>
            <div><dt>Install message</dt><dd><?=h($submission['message'] ?: '—')?></dd></div>
            <div><dt>Submitted by</dt><dd><?=h($submission['submitter_name'] ?: '—')?> <?php if ($submission['submitter_email']): ?>&lt;<?=h($submission['submitter_email'])?>&gt;<?php endif; ?></dd></div>
            <div><dt>From address</dt><dd><?=h($submission['remote_address'] ?: '—')?></dd></div>
          </dl>
          <form method="post" class="stack">
            <input type="hidden" name="csrf" value="<?=csrf()?>">
            <input type="hidden" name="form" value="submission_status">
            <input type="hidden" name="submission_id" value="<?=h($submission['id'])?>">
            <label>Status<select name="status"><?php foreach ($allowedStatuses as $statusOption): ?><option value="<?=h($statusOption)?>" <?=$submission['status']===$statusOption?'selected':''?>><?=h(submissionStatusLabel($statusOption))?></option><?php endforeach; ?></select></label>
            <button class="small">Update status</button>
          </form>
          <h3>Internal comments</h3>
          <div class="comment-thread">
            <?php foreach ($comments as $comment): ?>
              <div class="comment">
                <div class="comment-meta"><strong><?=h($comment['display_name'] ?: $comment['email'] ?: 'Former user')?></strong><span><?=h(str_replace(['T','Z'],[' ',' UTC'],$comment['created_at']))?></span></div>
                <p><?=nl2br(h($comment['body']))?></p>
              </div>
            <?php endforeach; ?>
            <?php if (!$comments): ?><p class="empty-row">No comments yet.</p><?php endif; ?>
          </div>
          <form method="post" class="stack">
            <input type="hidden" name="csrf" value="<?=csrf()?>">
            <input type="hidden" name="form" value="submission_comment">
            <input type="hidden" name="submission_id" value="<?=h($submission['id'])?>">
            <label>Add a comment (visible to plugin managers and administrators only)<textarea name="body" required></textarea></label>
            <button class="small">Post comment</button>
          </form>
        </div>
      </dialog>
    <?php endforeach; ?>
...
<?php if ($isDashboard && $canEditCatalogNow): ?>
  <?php if ($legacyDuplicates): ?>
    <dialog id="prune-duplicates-dialog">
      <form method="post" class="stack">
        <div class="dialog-title"><h2>Remove legacy duplicate entries</h2><button type="button" class="icon" data-close>×</button></div>
        <input type="hidden" name="csrf" value="<?=csrf()?>">
        <input type="hidden" name="form" value="catalog_prune_duplicates">
        ...
        <label>Minisign key password<input type="password" name="signing_password" autocomplete="current-password" required></label>
        <button class="danger">Remove and re-publish</button>
      </form>
    </dialog>
  <?php endif; ?>
  <?php if ($authorityMissing): ?>
    <dialog id="authority-missing-dialog">
      <div class="dialog-content">
        <div class="dialog-title"><div><span class="eyebrow">AUTHORITY</span><h2>Available from the authority catalog</h2></div><button type="button" class="icon" data-close>×</button></div>
        ...
        <div class="authority-missing-list">
          <?php foreach ($authorityMissing as $missingItem): $missingEntry = $missingItem['entry']; ?>
            <div class="authority-missing-row">
              ...
              <form method="post" class="authority-missing-form">
                <input type="hidden" name="csrf" value="<?=csrf()?>">
                <input type="hidden" name="form" value="authority_sync_entry">
                <input type="hidden" name="entry_id" value="<?=h($missingItem['id'])?>">
                <input type="password" name="signing_password" placeholder="Minisign password" autocomplete="current-password" required>
  ...
  <dialog id="repo-dialog">
    <form method="post" class="stack" data-spin-on-submit>
      <div class="dialog-title"><h2>Add GitHub repository</h2><button type="button" class="icon" data-close>×</button></div>
      <input type="hidden" name="csrf" value="<?=csrf()?>">
      <input type="hidden" name="form" value="repository_preview">
      <?php if ($prefillSubmission): ?><input type="hidden" name="submission_id" value="<?=h($prefillSubmission['id'])?>"><?php endif; ?>
      <label>Repository URL<input type="url" name="repository" placeholder="https://github.com/lefred/mariadb-plugin-vmstat" value="<?=h($prefillSubmission['repository'] ?? '')?>" required></label>
      <div class="form-grid">
        <label>Plugin types<input name="plugin_types" placeholder="auto-detect" value="<?=h($prefillSubmission['plugin_types'] ?? '')?>"></label>
        <label>License<input name="license" placeholder="auto-detect" value="<?=h($prefillSubmission['license'] ?? '')?>"></label>
        <label>Maturity<input name="maturity" placeholder="auto-detect" value="<?=h($prefillSubmission['maturity'] ?? '')?>"></label>
        <label>Name<input name="name" placeholder="auto-detect" value="<?=h($prefillSubmission['name'] ?? '')?>"></label>
        <label>Author<input name="author" placeholder="auto-detect (repository owner)"></label>
        <label>Architecture<input name="architecture" placeholder="standalone .so only, default x86_64"></label>
      </div>
      <label>Description<input name="description" placeholder="GitHub description" value="<?=h($prefillSubmission['description'] ?? '')?>"></label>
      <label>Dependencies<input name="dependencies" value="<?=h($prefillSubmission['dependencies'] ?? '')?>"></label>
      <label>Install message<textarea name="message"><?=h($prefillSubmission['message'] ?? '')?></textarea></label>
      ...
      <button data-loading-label="Detecting…">Detect and review</button>
    </form>
  </dialog>
  ...
  <?php if ($catalogPreview): ?>
    <dialog id="review-import-dialog" class="review-import-dialog">
      <div class="dialog-content">
        <div class="dialog-title"><div><span class="eyebrow">REVIEW BEFORE PUBLISHING</span><h2>Import from <?=h($catalogPreview['repository'])?></h2></div><button type="button" class="icon" data-close>×</button></div>
        ...
        <form method="post" class="stack" data-spin-on-submit>
          <input type="hidden" name="csrf" value="<?=csrf()?>">
          <input type="hidden" name="form" value="repository_confirm">
          <?php foreach ($catalogPreview['entries'] as $i => $entry): ?>
            <fieldset class="review-entry">
              <legend><label><input type="checkbox" name="entries[<?=$i?>][skip]" value="1"> Skip this entry</label></legend>
              <div class="form-grid">
          ...
          <button data-loading-label="Publishing…">Confirm and publish</button>
        </form>
        <form method="post" class="discard-form">
          <input type="hidden" name="csrf" value="<?=csrf()?>">
          <input type="hidden" name="form" value="repository_discard">
          <button type="submit" class="small secondary">Discard, don't publish</button>
        </form>
      </div>
    </dialog>
...
<?php if ($isDashboard && !$auth->isAuthenticated()): ?>
  <dialog id="submit-plugin-dialog">
    <form method="post" class="stack">
      <div class="dialog-title"><h2>Submit a plugin</h2><button type="button" class="icon" data-close>×</button></div>
      <input type="hidden" name="csrf" value="<?=csrf()?>">
      <input type="hidden" name="form" value="submit_plugin">
